...
/*
         ...
         * 假设有数组x[1...n]，其中最大的元素为max，最小元素为min，将左闭右开的实数区间[min，max)划分为n-1个等长的子区间（桶），
         * 每个子区间也是左闭右开的，
         * 我们用len来表示每个子区间的长度。除去max和min，剩下的n-2个数，每个数都属于其中一个桶。
         * 对于同一个桶的两个数，因为桶是左闭右开的，所以他们的距离肯定是小于len的。
         * 然后，关键的一点是，n-2个数放进n-1个桶，由抽屉原理可以知道，肯定有一个桶是空的，
         * 所以，距离最远的相邻的两个数，肯定是属于两个不同的桶。于是，我们可以把每个桶都扫描一次，
         * 相邻最远的两个数，必定其中一个是某个桶里的最大值，另一个是另一个桶里的最小值
        */
