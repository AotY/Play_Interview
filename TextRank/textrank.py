...
class TextRank:
    ...
    def solve(self):
        ...
        for _ in range(self.max_iter):
            ...
            for i in range(self.D):
                ...
                diff = m[-1] - self.vertex[i]
                if diff > max_diff:
                    max_diff = diff
            self.vertex = m  # vertex weight
    ...
    def top(self, limit):
        return list(map(lambda x: self.doc[x[0]], self.top))
...
class KeywordRank:
    ...
    def solve(self):
        ...
        for _ in range(self.max_iter):
            ...
            tmp = filter(lambda x: len(self.words[x[0]]) > 0, self.vertex.items())
